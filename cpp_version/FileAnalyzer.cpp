...
const std::unordered_set<std::string_view> FileAnalyzer::stopWords = {
    ...
    "s","t","can","will","just",
    "don't","should","now","d","ll","m","o","re","ve","y",
    "ain't","aren't","couldn't","didn't","doesn't","hadn't","hasn't","haven't","isn't","ma",
...
bool FileAnalyzer::isMicrosoftCompressedXML(const std::string &path) {
    static const std::unordered_set<std::string> MicrosoftCompressedXML_format_extension = {".docx", ".xlsx", ".pptx", ".epub"};
    return MicrosoftCompressedXML_format_extension.find(getExt(path)) != MicrosoftCompressedXML_format_extension.end();
}
